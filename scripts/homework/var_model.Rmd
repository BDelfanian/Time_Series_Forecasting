...
#### Stationarity Analysis, data_x

```{r stationarity check, data_x}

ts_vars <- data_x |> 
  select(-date) |> 
  names()

stationarity_results <- map_dfr(ts_vars, function(var) {
  
  series <- suppressWarnings(as.numeric(data_x[[var]]))
  series <- na.omit(series)
  
  if (length(series) < 10 || sd(series) == 0) {
    return(tibble(
      variable = var,
      ...
      stationary = NA,
      note = "Series too short or constant"
    ))
  }
  
  adf  <- suppressWarnings(tseries::adf.test(series))
  kpss <- suppressWarnings(tseries::kpss.test(series))
  
  stationary_flag <- (adf$p.value < 0.05) & (kpss$p.value > 0.05)
  
  tibble(
    variable = var,
    adf_statistic  = as.numeric(adf$statistic),
    adf_p_value    = adf$p.value,
    kpss_statistic = as.numeric(kpss$statistic),
    kpss_p_value   = kpss$p.value,
    stationary     = stationary_flag,
    note = ifelse(stationary_flag,
                  "Stationary (ADF reject, KPSS not reject)",
                  "Non-stationary or inconclusive")
  )
})

stationarity_results[stationarity_results$stationary == FALSE, ]
```
...
#### Stationarity Analysis, y_xx

```{r stationarity check, y_xx}
...
stationarity_results <- map_dfr(ts_vars, function(var) {
  ...
  series <- suppressWarnings(as.numeric(y_xx[[var]]))
  series <- na.omit(series)
...
#### Stationarity Analysis, top5_freq

```{r stationarity check, tot5_freq}

ts_vars <- y_xx |> 
  select(top5_freq$Series) |> 
  names()
