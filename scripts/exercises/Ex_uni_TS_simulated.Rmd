...
## Packages
We will be using stats, ggplot2, ggpubr, gridExtra and forecast packages (see also tseries package!). 
...
1. How does the pattern of the series change when $\phi_1$ ↑?
...
# TEST:  t-stat for gamma, but with "special" critical values 
# DECISION: If p-value is below (0.1/0.05/0.01) we reject H0 in favor of stationarity
res <- aTSA::adf.test(y,nlag = NULL, output = FALSE) 
print(res$type1)
print("The p-value at lag length 6 [default= floor(4*(length(x)/100)^(2/9))] is less than 0.1/0.05/0.01. Therefore we reject H0, which states that the series is non-stationary.")
```
...
res <- aTSA::adf.test(y,nlag = NULL, output = FALSE) 
print(res$type2)
print("The p-value at lag length 7 [default= floor(4*(length(x)/100)^(2/9))] is less than 0.1/0.05/0.01. Therefore we reject H0, which states that the series is non-stationary.")
```
...
# The following holds in this package:
...
# Type 1 - no drift and no deteministic trend
# Type 2 - model with drift
# Type 3 - model with drift and time trend
res <- aTSA::kpss.test(y,lag.short = TRUE, output = FALSE)
# H0: Series is stationary/Series is trend stationary
print("For all model types we cannot reject H0, which states that the series is stationary.")
